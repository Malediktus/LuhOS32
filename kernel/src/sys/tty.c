...
static char_device_t *tty_char_device;

void init_tty(char_device_t *cdev)
{
    tty_char_device = cdev;
}
...
{
    if (row < char_device_get_height(tty_char_device))
    {
        return;
    }

    char_device_scroll_line(tty_char_device);
    col = 0;
    row--;
}
...
{
    uint32_t result = char_device_write_char(tty_char_device, col, row, current_color, c);
    if (result != EOK)
    {
        while (true)
        {
    ...
    col++;
    if (col >= char_device_get_width(tty_char_device))
    {
        col = 0;
        row++;
        scroll_screen();
...
void clear_tty()
{
    for (uint32_t y = 0; y < char_device_get_height(tty_char_device); y++)
    {
        for (uint32_t x = 0; x < char_device_get_width(tty_char_device); x++)
        {
            uint32_t result = char_device_write_char(tty_char_device, x, y, TTY_COLOR(TTY_WHITE, TTY_BG_BLACK), ' ');
            if (result != EOK)
            {
                while (true)
                {
...
void kprint_at_color(const char *message, uint32_t _col, uint32_t _row, uint8_t color)
{
    ...
    for (uint32_t i = 0; message[i] != 0; i++)
    {
        ...
        else if (message[i] == '\b')
        {
            ...
            {
                row--;
                col = char_device_get_width(tty_char_device) - 1;
            }

            uint32_t result = char_device_write_char(tty_char_device, col, row, current_color, ' ');
            if (result != EOK)
            {
                while (true)
                {
